defmodule KyuubikiSdk.Error do
  ...
  def model_collaboration_validation(errors) when is_list(errors) do
    %__MODULE__{
      type: :validation,
      message: "model collaboration validation failed:\n- " <> Enum.join(errors, "\n- ")
    }
  end

  def model_research_execution(errors) when is_list(errors) do
    %__MODULE__{
      type: :validation,
      message: "model research execution failed:\n- " <> Enum.join(errors, "\n- ")
    }
  end
