    ...
    pub fn validate(&self) -> SdkResult<()> {
        ...
        for (index, node_id) in self.entry_nodes.iter().enumerate() {
            if !node_ids.contains(node_id.as_str()) {
                errors.push(format!(
                    "graph.entry_nodes[{index}] references unknown node {:?}",
                    node_id
                ));
            }
        }
        for (index, node_id) in self.output_nodes.iter().enumerate() {
            if !node_ids.contains(node_id.as_str()) {
                errors.push(format!(
                    "graph.output_nodes[{index}] references unknown node {:?}",
                    node_id
                ));
            }
        }
        ...
        for (index, edge) in self.edges.iter().enumerate() {
            ...
            if source.is_none() {
                errors.push(format!(
                    "{path}.from references unknown output port {:?}",
                    source_key
                ));
            }
            if target.is_none() {
                errors.push(format!(
                    "{path}.to references unknown input port {:?}",
                    target_key
                ));
            }
            if let Some(source_port) = source {
                if source_port.artifact_type != edge.artifact_type {
                    errors.push(format!(
                        "{path}.artifact_type does not match source output port artifact_type"
                    ));
                }
            }
            if let Some(target_port) = target {
                if target_port.artifact_type != edge.artifact_type {
                    errors.push(format!(
                        "{path}.artifact_type does not match target input port artifact_type"
                    ));
                }
            }
            ...
            if let Some(dataset_value) = edge.dataset_value.as_deref() {
                if !dataset_ids.is_empty() && !dataset_ids.contains(dataset_value) {
                    errors.push(format!(
                        "{path}.dataset_value {:?} is not declared in graph.dataset_contract",
                        dataset_value
                    ));
                }
            }
...
fn collect_ports<'a>(
    ...
    for (index, port) in ports.iter().enumerate() {
        ...
        if let Some(dataset_value) = port.dataset_value.as_deref() {
            if !dataset_ids.is_empty() && !dataset_ids.contains(dataset_value) {
                errors.push(format!(
                    "{port_path}.dataset_value {:?} is not declared in graph.dataset_contract",
                    dataset_value
                ));
            }
        }
