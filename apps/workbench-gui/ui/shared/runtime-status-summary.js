import { buildRuntimeStatusModel, selectRuntimeStatusModelDetail, selectRuntimeStatusModelFilter, } from "./runtime-status-model.js";
...
function normalizeMetric(value, fallback = "--", ..._unused) {
...
function buildRuntimeStateCard(runtime) {
    ...
    return `
    <article class="desktop-runtime-plane__card ${runtime.isSelected ? "desktop-runtime-plane__card--selected" : ""} desktop-runtime-plane__card--deployment-${escapeHtml(deployment)} desktop-runtime-plane__card--type-${escapeHtml(runtimeType)} desktop-runtime-plane__card--state-${escapeHtml(state)} ${authority ? `desktop-runtime-plane__card--authority-${escapeHtml(authority)}` : ""}" ${selectionKey ? `data-runtime-detail="${escapeHtml(selectionKey)}"` : ""} tabindex="${selectionKey ? "0" : "-1"}">
      <span class="desktop-runtime-plane__label">${escapeHtml(label)}</span>
      <strong class="desktop-runtime-plane__value desktop-shell-state desktop-shell-state--${escapeHtml(state)}">${escapeHtml(sentenceCase(state))}</strong>
      <span class="desktop-runtime-plane__badge">${escapeHtml(badge)}</span>
      <span class="desktop-runtime-plane__note">${escapeHtml(note)}</span>
    </article>
  `;
}
function buildTopologyStat(label, value) {
    return `
...
function buildTopologyItem(item) {
    ...
    return `
    <div class="desktop-mesh-topology__item ${item.isSelected ? "desktop-mesh-topology__item--selected" : ""}" ${selectionKey ? `data-runtime-detail="${escapeHtml(selectionKey)}"` : ""} tabindex="${selectionKey ? "0" : "-1"}">
      <div class="desktop-mesh-topology__item-head">
        ...
        <div class="desktop-mesh-topology__item-meta">
          ${meta
        .map((entry) => `<span class="desktop-runtime-plane__pill"><strong>${escapeHtml(normalizeMetric(entry.label, ""))}</strong>${escapeHtml(normalizeMetric(entry.value, ""))}</span>`)
        .join("")}
        </div>
      </div>
...
function buildTopologySection(section) {
...
function buildMeshPanel(panel) {
    const pills = Array.isArray(panel.pills) ? panel.pills : [];
    const items = Array.isArray(panel.items) ? panel.items : [];
    const classes = ["desktop-runtime-plane__mesh-panel"];
    if (panel.wide)
        classes.push("desktop-runtime-plane__mesh-panel--wide");
    const selectionKey = normalizeMetric(panel.selectionKey, "");
    return `
    <section class="${classes.join(" ")} ${panel.isSelected ? "desktop-runtime-plane__mesh-panel--selected" : ""}" ${selectionKey ? `data-runtime-detail="${escapeHtml(selectionKey)}"` : ""} tabindex="${selectionKey ? "0" : "-1"}">
      ...
      ${pills.length
        ? `<div class="desktop-runtime-plane__pill-row">${pills
            .map((pill) => `<span class="desktop-runtime-plane__pill"><strong>${escapeHtml(normalizeMetric(pill.label, ""))}</strong>${escapeHtml(normalizeMetric(pill.value, ""))}</span>`)
            .join("")}</div>`
        : ""}
      ${items.length
        ? `<div class="desktop-runtime-plane__mesh-list">${items
            .map((item) => `
                  <article class="desktop-runtime-plane__mesh-item ${item.isSelected ? "desktop-runtime-plane__mesh-item--selected" : ""}" ${item.selectionKey ? `data-runtime-detail="${escapeHtml(item.selectionKey)}"` : ""} tabindex="${item.selectionKey ? "0" : "-1"}">
...
function buildFilterRow(filters, selectedValue) {
    ...
    <div class="desktop-runtime-plane__filters">
      ${filters
        ...
        return `<button class="desktop-runtime-plane__filter${active ? " is-active" : ""}" data-runtime-filter="${escapeHtml(value)}" type="button">${escapeHtml(normalizeMetric(filter.label, value || "Filter"))}</button>`;
    })
        .join("")}
    </div>
  `;
...
function applySelectionDecorators(model) {
...
function buildDetailPane(detailSelection) {
...
function normalizeRenderInputs(payload, options = {}) {
    ...
    if (summaryPayload || healthPayload) {
        ...
        return {
            ...
            options: isRecord(options) && ("onFilterChange" in options || "healthPayload" in options)
                ? renderOptions
                : {},
        };
    }
    return {
        model: normalizePayload(payload),
        options: renderOptions,
    };
}
export function formatRuntimeStatusReport(payload, options = {}) {
...
export function renderRuntimeStatusPlane(target, payload, options = {}) {
    if (!target)
        return;
  ...
  `;
    target.querySelectorAll("[data-runtime-filter]").forEach((button) => {
    ...
    target.querySelectorAll("[data-runtime-detail]").forEach((node) => {
