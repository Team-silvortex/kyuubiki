...
function classifyStudyDomain(kind: StudyKind): "mechanical" | "thermal" | "thermo_mechanical" {
  switch (kind) {
    case "heat_bar_1d":
...
function classifyStudyFamily(kind: StudyKind): "axial_and_springs" | "beams_and_frames" | "trusses" | "planes" {
  switch (kind) {
    case "axial_bar_1d":
    case "heat_bar_1d":
    case "thermal_bar_1d":
    ...
    return JSON.stringify(
      {
        ...
        model_schema_version: MODEL_SCHEMA_VERSION,
        name: payload.name,
        analysis_metadata: buildAnalysisMetadata("axial_bar_1d", payload),
        material: payload.material,
