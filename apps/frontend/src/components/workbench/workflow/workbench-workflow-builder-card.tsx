...
import type { WorkflowCatalogEntryArtifact, WorkflowDatasetAxis, WorkflowDatasetValueInfo, WorkflowGraphDefinition } from "@/lib/api";
...
import { builtInWorkflowSampleInputArtifacts } from "@/components/workbench/workflow/workbench-workflow-sample-inputs"; import { WorkbenchWorkflowTopologyCard } from "@/components/workbench/workflow/workbench-workflow-topology-card";
...
export function WorkbenchWorkflowBuilderCard({
  ...
  recentRunStatus,
  latestRun,
  protocolAgents = EMPTY_PROTOCOL_AGENTS,
  frontendRuntimeMode,
  ...
  const [inputFocusRequest, setInputFocusRequest] = useState<string | null>(null);
  ...
  useEffect(() => {
    if (inputFocusRequest === null || builderStage !== "contracts" || contractView !== "inputs") return;
    const input = builderRootRef.current?.querySelector<HTMLTextAreaElement>(
      `[data-workflow-input-artifact="${CSS.escape(inputFocusRequest)}"]`,
    );
    if (!input) return;
    input.focus({ preventScroll: true });
    input.scrollIntoView({ block: "center" });
    setInputFocusRequest(null);
  }, [builderStage, contractView, inputFocusRequest]);
  const parsedDraftInputs = useMemo(() => parseWorkflowInputArtifactTexts(draftInputTexts, selectedEntryInputs), [draftInputTexts, selectedEntryInputs]);
  ...
  const draftBlockerMessage = parsedDraftInputs.invalidKeys.length > 0
    ? `${parsedDraftInputs.invalidKeys[0]}: ${labels.runDraftInvalidInputsLabel}`
    : validationIssues[0]?.message ?? null;
  ...
  function locateDraftBlocker() {
    const inputKey = parsedDraftInputs.invalidKeys[0];
    if (inputKey !== undefined) {
      setBuilderStage("contracts");
      setContractView("inputs");
      setInputFocusRequest(inputKey);
      return;
    }
    const issue = validationIssues[0];
    if (issue?.locate) locateBuilderIssue(issue.locate);
    else setBuilderStage("validation");
  }
  ...
  function locateBuilderIssue(locate: WorkflowBuilderLocateTarget) {
    ...
    window.requestAnimationFrame(() => {
      locateWorkflowBuilderIssue({ locate, builderRootRef, selectedDatasetValues, selectedEntryInputs, selectedOutputArtifacts, resetBuilderFocus, setFocusedNodeId, setFocusedEdgeId, setSelectedDatasetValueId, setFocusedDatasetValueId, setHighlightDatasetEditor, setFocusedArtifactKey });
    ...
    <section className="sidebar-card sidebar-card--compact workflow-builder-shell" data-workflow-builder-shell="builder" data-workflow-builder-stage={builderStage} ref={builderRootRef}>
      ...
      {showDeferredPanels && (builderStage === "validation" || (builderStage === "contracts" && contractView !== "inputs" && contractView !== "storage")) ? (
        <WorkbenchWorkflowBuilderDeferredPanelStack
          ...
          onRestoreSnapshot={restoreSnapshot} onSaveLocalMetadata={saveCurrentLocalWorkflowMetadata} onScanPackageResiduals={scanPackageResiduals}
          packageResiduals={packageResiduals} protocolAgents={protocolAgents} recentFixSummary={recentFixSummary} removeArtifact={removeArtifact} removeDatasetAxis={removeDatasetAxis} removeSelectedDatasetValue={removeSelectedDatasetValue}
          selectedDatasetContract={selectedDatasetContract} selectedDatasetValue={selectedDatasetValue} selectedDatasetValueId={selectedDatasetValueId} selectedDatasetValues={selectedDatasetValues} selectedEdges={selectedEdges} selectedEntryInputs={selectedEntryInputs} selectedNodes={selectedNodes} selectedOutputArtifacts={selectedOutputArtifacts}
          setSelectedDatasetValueId={setSelectedDatasetValueId} snapshots={savedSnapshots} traceFocusBranchNodeId={traceFocusBranchNodeId} traceFocusBranchOutputId={traceFocusBranchOutputId}
          updateArtifact={updateArtifact} updateDatasetAxis={updateDatasetAxis} updateDatasetValue={updateDatasetValue} updateEdgeDatasetValue={updateEdgeDatasetValue} updateNodePortDatasetValue={updateNodePortDatasetValue}
          validationIssues={validationIssues} workflow={selectedWorkflow}
        />
      ) : null}
    </section>
  );
