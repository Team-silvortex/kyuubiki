...
function summarizeAgentHealth(agent) {
    ...
    if (typeof agent.health_score === "number")
        parts.push(`health ${agent.health_score}`);
...
function buildMeshRuntimeEntries(healthPayload) {
    ...
    return solverAgents.map((agent, index) => {
        ...
        return {
            ...
            status: typeof agent.cooldown_remaining_ms === "number" && agent.cooldown_remaining_ms > 0
                ? "degraded"
                : sessionState,
    ...
    return [
        {
            ...
            items: managedGroups.map((group) => ({
                ...
                meta: [
                    ...
                    {
                        label: "Sessions",
                        value: Array.isArray(group.session_ids) && group.session_ids.length
...
function buildRuntimeFilters(meshHealth) {
    ...
    const degradedCount = asArray(meshHealth.solver_agents).filter((agent) => typeof agent.cooldown_remaining_ms === "number" && agent.cooldown_remaining_ms > 0).length;
    const filters = [{ value: "all", label: "All" }];
    if (managedCount > 0)
        filters.push({ value: "orch-managed", label: "Managed" });
    if (offlineCount > 0)
        filters.push({ value: "offline-mesh", label: "Offline mesh" });
    if (degradedCount > 0)
        filters.push({ value: "degraded", label: "Degraded" });
    return filters;
}
...
function applyModelFilter(model, filterValue) {
    if (!filterValue || filterValue === "all")
        return { ...model, selectedFilter: "all" };
    return {
        ...model,
...
export function buildRuntimeStatusModel(summaryPayload, healthPayload) {
    ...
    const totalMeshAgents = meshHealth?.remote_solver_registry?.total_agents ??
...
function collectSelectableEntries(model) {
    return [
        ...
        ...model.topology.flatMap((section, index) => section.items.map((item, itemIndex) => ({
