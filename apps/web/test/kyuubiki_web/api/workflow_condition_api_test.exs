defmodule KyuubikiWeb.Api.WorkflowConditionApiTest do
  ...
  test "runs a diagnostics bundle transform through the workflow graph" do
    conn =
      :post
      |> conn(
        "/api/v1/workflows/graph/run",
          ...
          "graph" => %{
            ...
            "edges" => [
              ...
              %{
                ...
                "to" => %{"node" => "bundle_output", "port" => "result"},
                "artifact_type" => "artifact/json"
              }
            ]
          },
    ...
    assert get_in(payload, ["artifacts", "bundle.result", "bundle_source_count"]) == 2
    assert get_in(payload, ["artifacts", "bundle.result", "bundle_total_node_count"]) == 7

    assert get_in(payload, ["artifacts", "bundle.result", "bundle_domains"]) == [
             "electrostatic",
             "thermal"
           ]
  end
