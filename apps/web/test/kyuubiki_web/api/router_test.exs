defmodule KyuubikiWeb.Playground.RouterTest do
  ...
  test "submits a catalog workflow as an asynchronous job" do
    ...
    conn =
      ...
      |> conn(
        ...
        Jason.encode!(%{"input_artifacts" => heat_to_thermo_quad_input_artifacts()})
      )
      |> put_req_header("content-type", "application/json")
      |> Router.call(@opts)
  ...
  defp heat_to_thermo_quad_input_artifacts do
    %{
      "heat_model" => %{
        "nodes" => [
          %{
  ...
  defp start_fake_agent_sessions(frame_sessions) when is_list(frame_sessions) do
    parent = self()
