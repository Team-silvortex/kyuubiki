defmodule KyuubikiWeb.Playground.AgentSessionState do
  ...
  def state(agent) when is_map(agent) do
    case {Map.get(agent, :control_mode) || Map.get(agent, "control_mode"),
          Map.get(agent, :orch_session_id) || Map.get(agent, "orch_session_id")} do
      {"offline_mesh", _} ->
        "offline_mesh"

      {"orch_managed", session_id} when is_binary(session_id) and session_id != "" ->
        "orch_session_bound"
  ...
  def transition(nil, next_agent, source),
    do: new_transition("unregistered", state(next_agent), source, "registered", next_agent)
    ...
    cond do
      current_state != next_state ->
        new_transition(
          ...
          next_state,
          source,
          transition_reason(current_state, next_state),
          next_agent
        )
  ...
  defp changed_session_id?(current, next_agent) do
    ...
    next_session_id =
      Map.get(next_agent, :orch_session_id) || Map.get(next_agent, "orch_session_id")

    current_session_id != next_session_id
  end

  defp transition_reason("orch_bound_pending_session", "orch_session_bound"),
    do: "session_attached"
  ...
  defp transition_reason("offline_mesh", "orch_bound_pending_session"),
    do: "orchestrator_attached"
