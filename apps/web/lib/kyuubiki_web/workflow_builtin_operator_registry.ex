defmodule KyuubikiWeb.WorkflowBuiltinOperatorRegistry do
  ...
  alias KyuubikiWeb.WorkflowSummaryOperatorSpecs

  @transform_specs [
    {"transform.evaluate_acoustic_guard", "evaluate_acoustic_guard",
     "Evaluate acoustic summaries against visible warn/block threshold rules.",
     ["transform", "physics", "acoustic", "guard", "decision", "headless_safe"]},
    {"transform.benchmark_acoustic_pair", "benchmark_acoustic_pair",
     "Compare two acoustic candidates with weighted min/max criteria.",
     ["transform", "physics", "acoustic", "benchmark", "decision", "headless_safe"]},
    {"transform.score_acoustic_quality", "score_acoustic_quality",
     "Score acoustic response summaries against SPL, intensity, pressure, and damping targets.",
     ["transform", "physics", "acoustic", "quality", "optimization", "headless_safe"]},
    {"transform.evaluate_modal_guard", "evaluate_modal_guard",
     "Evaluate modal summaries against frequency, mass, period, or participation threshold rules.",
     ["transform", "physics", "modal", "guard", "decision", "headless_safe"]},
    {"transform.benchmark_modal_pair", "benchmark_modal_pair",
     "Compare two modal candidates with weighted min/max criteria.",
     ["transform", "physics", "modal", "benchmark", "decision", "headless_safe"]},
    {"transform.score_modal_quality", "score_modal_quality",
     "Score modal summaries against frequency, mass, participation, and spread targets.",
     ["transform", "physics", "modal", "quality", "optimization", "headless_safe"]},
    {"transform.evaluate_dynamic_guard", "evaluate_dynamic_guard",
     "Evaluate dynamic response summaries against frequency, displacement, velocity, acceleration, or force threshold rules.",
     ["transform", "physics", "dynamic", "guard", "decision", "headless_safe"]},
    {"transform.benchmark_dynamic_pair", "benchmark_dynamic_pair",
     "Compare two dynamic response candidates with weighted min/max criteria.",
     ["transform", "physics", "dynamic", "benchmark", "decision", "headless_safe"]},
    {"transform.score_dynamic_quality", "score_dynamic_quality",
     "Score dynamic response summaries against resonance, displacement, acceleration, and force targets.",
     ["transform", "physics", "dynamic", "quality", "optimization", "headless_safe"]},
    {"transform.evaluate_structural_guard", "evaluate_structural_guard",
     "Evaluate structural summaries against displacement, stress, mass, or stiffness threshold rules.",
     ["transform", "physics", "structural", "guard", "decision", "headless_safe"]},
    {"transform.benchmark_structural_pair", "benchmark_structural_pair",
     "Compare two structural candidates with weighted min/max criteria.",
     ["transform", "physics", "structural", "benchmark", "decision", "headless_safe"]},
    {"transform.score_structural_quality", "score_structural_quality",
     "Score structural summaries against displacement, stress, mass, and stiffness targets.",
     ["transform", "physics", "structural", "quality", "optimization", "headless_safe"]},
    {"transform.score_thermal_quality", "score_thermal_quality",
     "Score thermal and thermo-mechanical summaries against temperature, flux, and stress targets.",
     ["transform", "physics", "thermal", "quality", "optimization", "headless_safe"]},
    {"transform.score_electrostatic_quality", "score_electrostatic_quality",
     "Score electrostatic summaries against field, energy-density, and potential-span targets.",
     ["transform", "physics", "electrostatic", "quality", "optimization", "headless_safe"]},
    {"transform.score_magnetostatic_quality", "score_magnetostatic_quality",
     "Score magnetostatic summaries against field, flux, energy-density, and current targets.",
     ["transform", "physics", "magnetostatic", "quality", "optimization", "headless_safe"]},
    {"transform.score_cfd_quality", "score_cfd_quality",
     "Score CFD summaries against divergence, Reynolds, dissipation, velocity, and pressure targets.",
     ["transform", "physics", "cfd", "quality", "optimization", "headless_safe"]},
    {"transform.evaluate_transport_guard", "evaluate_transport_guard",
     "Evaluate transport summaries against flux, Peclet, concentration, or source threshold rules.",
     ["transform", "physics", "transport", "guard", "decision", "headless_safe"]},
    {"transform.benchmark_transport_pair", "benchmark_transport_pair",
     "Compare two transport candidates with weighted min/max criteria.",
     ["transform", "physics", "transport", "benchmark", "decision", "headless_safe"]},
    {"transform.score_transport_quality", "score_transport_quality",
     "Score transport summaries against flux, Peclet, concentration-span, and source targets.",
     ["transform", "physics", "transport", "quality", "optimization", "headless_safe"]},
    ...
    {"transform.plan_material_experiments", "material_experiment_plan",
     "Convert scored material rankings into a prioritized experiment shortlist for automated material exploration loops.",
     [
       "transform",
       "material",
       "experiment_plan",
    ...
    {"transform.analyze_material_experiment_results", "material_experiment_result_analysis",
     "Analyze observed material experiment results against planned scores and emit validated candidate rankings.",
     [
       "transform",
       "material",
       "experiment_result",
    ...
    {"transform.decide_material_iteration", "material_iteration_decision",
     "Decide whether a material exploration loop should stop, continue, or replan from validated experiment results.",
     [
       "transform",
       "material",
       "iteration_decision",
    ...
    {"transform.prepare_material_next_round_request", "material_next_round_request",
     "Prepare a standard next-round material exploration request from an iteration decision for orchestration handoff.",
     [
       "transform",
       "material",
       "next_round",
    ...
    {"transform.build_material_exploration_snapshot", "material_exploration_snapshot",
     "Build a durable material exploration state snapshot from a next-round orchestration request.",
     [
       "transform",
       "material",
       "snapshot",
    ...
    {"transform.extract_material_pareto_frontier", "material_pareto_frontier",
     "Extract the Pareto frontier from material candidates using configured min/max objectives and feasibility flags.",
     ["transform", "material", "pareto", "multi_objective", "optimization", "headless_safe"]},
  ...
  def list do
    heat_to_thermo_bridge_specs() ++
      ...
      Enum.map(WorkflowSummaryOperatorSpecs.transform_specs(), &transform_spec/1) ++
