defmodule KyuubikiWeb.WorkflowTemplateCustomEntries do
  ...
  defp electrostatic_to_heat_quad_entry do
    custom_entry(
      ...
      graph_from_chain(
        ...
        [
          solve_node("solve_electrostatic", "solve.electrostatic_plane_quad_2d", "study_model/electrostatic_plane_quad_2d", "result/electrostatic_plane_quad_2d"),
          transform_node(
          ...
          solve_node("solve_heat", "solve.heat_plane_quad_2d", "study_model/heat_plane_quad_2d", "result/heat_plane_quad_2d"),
          extract_node("extract_summary", "result/heat_plane_quad_2d", ["max_temperature", "max_heat_flux"]),
          export_node(),
          output_node()
        ]
      ),
  ...
  defp electrostatic_plane_quad_entry do
    custom_entry(
      ...
      graph_from_chain(
        ...
        [
          solve_node("solve_electrostatic", "solve.electrostatic_plane_quad_2d", "study_model/electrostatic_plane_quad_2d", "result/electrostatic_plane_quad_2d"),
          extract_node("extract_summary", "result/electrostatic_plane_quad_2d", ["max_potential", "max_electric_field", "max_flux_density"]),
          export_node(),
          output_node()
        ]
      ),
    ...
    custom_entry(
      ...
      graph_from_chain(
        ...
        [
          ...
          solve_node("solve_thermo", "solve.thermal_plane_quad_2d", "study_model/thermal_plane_quad_2d", "result/thermal_plane_quad_2d"),
  ...
  defp electrostatic_to_heat_triangle_entry do
    custom_entry(
      ...
      graph_from_chain(
        ...
        [
          solve_node("solve_electrostatic", "solve.electrostatic_plane_triangle_2d", "study_model/electrostatic_plane_triangle_2d", "result/electrostatic_plane_triangle_2d"),
          transform_node(
            "bridge_field_to_heat",
          ...
          solve_node("solve_heat", "solve.heat_plane_triangle_2d", "study_model/heat_plane_triangle_2d", "result/heat_plane_triangle_2d"),
          extract_node("extract_summary", "result/heat_plane_triangle_2d", ["max_temperature", "max_heat_flux"]),
          export_node(),
          output_node()
        ]
      ),
      [
        %{
          ...
          "description" => "Electrostatic plane triangle study model used as the workflow entry artifact."
        }
      ],
      [
        %{
          "node_id" => "json_output",
          "artifact_type" => "export/json",
          "description" => "JSON-encoded heat triangle summary after bridging the electrostatic field."
        }
      ]
    )
  end
  ...
  defp electrostatic_heat_thermo_triangle_entry do
    custom_entry(
      ...
      ["electrostatic", "heat", "thermal", "thermo_mechanical", "workflow_bridge", "triangle", "summary", "2d"],
      graph_from_chain(
        "workflow.electrostatic-heat-thermo-triangle-summary-json",
        "Electrostatic heat thermo triangle summary JSON",
        "electrostatic_plane_triangle_model",
        ...
        [
          ...
          solve_node("solve_thermo", "solve.thermal_plane_triangle_2d", "study_model/thermal_plane_triangle_2d", "result/thermal_plane_triangle_2d"),
          extract_node("extract_summary", "result/thermal_plane_triangle_2d", ["max_displacement", "max_stress", "max_temperature_delta"]),
          export_node(),
          output_node()
        ]
      ),
      [
        %{
          "node_id" => "electrostatic_plane_triangle_model",
          "artifact_type" => "study_model/electrostatic_plane_triangle_2d",
          "description" => "Electrostatic plane triangle study model used as the coupled workflow entry artifact."
        }
      ],
      [
        %{
  ...
  defp graph_from_chain(id, name, entry_node_id, entry_artifact_type, chain_nodes) do
    nodes = [
      %{"id" => entry_node_id, "kind" => "input", "outputs" => [%{"id" => "model", "artifact_type" => entry_artifact_type}]}
      | chain_nodes
    ]
