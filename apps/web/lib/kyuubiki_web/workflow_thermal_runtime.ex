defmodule KyuubikiWeb.WorkflowThermalRuntime do
  ...
  def extract_thermal_result_diagnostics(payload, config)
    ...
    with {:ok, nodes} <- fetch_list(payload, Map.get(config, "node_source", "nodes")),
      ...
      prefix = Stats.normalize_prefix(Map.get(config, "output_prefix", "thermal"))
      ...
      diagnostics =
        ...
        |> Stats.merge_diagnostic_contract("thermal", "thermal_result", prefix, [
          "temperature",
          "heat_load",
          "gradient",
          "flux"
        ])
        |> Stats.maybe_merge_temperature_stats(prefix, temperature_nodes)
        |> Stats.maybe_merge_load_stats(prefix, heat_load_nodes)
        |> Stats.maybe_merge_peak_vector(prefix, "gradient", gradient_peak)
        |> Stats.maybe_merge_peak_vector(prefix, "flux", flux_peak)
  ...
  def extract_thermo_result_diagnostics(payload, config)
    ...
    with {:ok, nodes} <- fetch_list(payload, Map.get(config, "node_source", "nodes")),
         {:ok, elements} <- fetch_list(payload, Map.get(config, "element_source", "elements")) do
      prefix = Stats.normalize_prefix(Map.get(config, "output_prefix", "thermo"))

      delta_field =
        Map.get(config, "temperature_delta_field") ||
          Map.get(config, "temperature_field", "temperature_delta")
      ...
      diagnostics =
        ...
        |> Stats.merge_diagnostic_contract("thermo_mechanical", "thermo_result", prefix, [
          "temperature_delta",
          "displacement",
          "stress"
        ])
        |> Stats.maybe_merge_temperature_delta_stats(prefix, delta_nodes)
        |> Stats.maybe_merge_peak_magnitude(prefix, "displacement", displacement_peak)
        |> Stats.maybe_merge_peak_scalar(prefix, "stress", stress_peak)
        |> Stats.maybe_merge_peak_scalar(prefix, "thermal_strain", thermal_strain_peak)
        |> Stats.maybe_merge_peak_scalar(prefix, "mechanical_strain", mechanical_strain_peak)
        |> Stats.maybe_merge_peak_scalar(prefix, "total_strain", total_strain_peak)
  ...
  def benchmark_coupled_heat_pair(%{"left" => left, "right" => right}, config)
    ...
    if criteria == [] do
      ...
      left_label = Stats.normalize_prefix(Map.get(config, "left_label", "left"))
      right_label = Stats.normalize_prefix(Map.get(config, "right_label", "right"))
  ...
  defp resolve_thermo_strain_peak(elements, base_field)
       when is_list(elements) and is_binary(base_field) do
  ...
  defp peak_displacement_entry(nodes, config) do
    peak_vector_entry(
      ...
      Map.get(config, "displacement_magnitude_field") ||
        Map.get(config, "displacement_field", "displacement_magnitude")
    ) ||
      peak_vector_entry(
        nodes,
