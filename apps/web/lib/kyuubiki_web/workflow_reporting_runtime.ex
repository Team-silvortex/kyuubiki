defmodule KyuubikiWeb.WorkflowReportingRuntime do
  ...
  defdelegate extract_field_statistics(payload, config),
    to: KyuubikiWeb.WorkflowFieldRuntime,
    as: :statistics

  defdelegate extract_field_hotspots(payload, config),
    to: KyuubikiWeb.WorkflowFieldRuntime,
    as: :hotspots
  ...
  def export_diagnostics_bundle_markdown(payload, config)
    ...
    lines =
      [
        ...
        "- Total Nodes: #{markdown_value(Map.get(payload, "bundle_total_node_count"))}",
        "- Total Elements: #{markdown_value(Map.get(payload, "bundle_total_element_count"))}"
      ] ++
          ...
          Enum.flat_map(Enum.take(items, max_items), fn
            %{} = item ->
              [
                ...
                "- Nodes: #{markdown_value(Map.get(item, "node_count"))}",
                "- Elements: #{markdown_value(Map.get(item, "element_count"))}",
