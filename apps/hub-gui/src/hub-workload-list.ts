...
type RenderWorkloadFiltersParams = {
  elements: WorkloadListElements;
  state: WorkloadListState;
};

type SourceBadge = [string, string];

type RenderHubWorkloadLibraryListParams = {
  elements: WorkloadListElements;
  entries: HubWorkloadEntry[];
  state: WorkloadListState;
  renderWorkloadFilters: (params?: RenderWorkloadFiltersParams) => void;
  renderEmptyHistoryState: (container: HTMLElement, message: string) => void;
  emptyMessage: string;
  filterEmptyMessage: string;
  matchesWorkloadFilter: (entry: HubWorkloadEntry) => boolean;
  matchesWorkloadSearchQuery: (entry: HubWorkloadEntry) => boolean;
  workloadSourceBadge: (entry: HubWorkloadEntry) => SourceBadge;
  ...
  restoredWorkloadContextMessage: string;
  loadedWorkloadContextMessage: string;
  removedWorkloadMessage: string;
  renderAssistantContext: () => void;
  renderHubAssistantLocalCards: () => void;
  openWorkloadInWorkbench: (entry: HubWorkloadEntry) => Promise<unknown>;
  formatHubOperatorError: (error: unknown, options?: Record<string, unknown>) => string;
  runAction: (action: string) => Promise<unknown> | unknown;
  downloadRemoteWorkloadBundle: (entry: HubWorkloadEntry) => Promise<unknown>;
  attachCurrentBundleToWorkload: (entry: HubWorkloadEntry) => Promise<unknown>;
...
function searchTokens(query: unknown): string[] {
  return String(query || "")
    .trim()
    .toLowerCase()
    .split(/\s+/u)
...
export function renderHubWorkloadLibraryList(params: RenderHubWorkloadLibraryListParams): void {
  const {
    elements,
    entries,
    state,
