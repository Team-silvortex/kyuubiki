import type { HubProjectActionDeps } from "./hub-action-contexts.js";

export async function runHubProjectAction(
  action: string,
  options: HubProjectActionDeps,
): Promise<boolean> {
  switch (action) {
    case "open-workbench":
    ...
    case "project-create":
      await options.runProjectBundleAction({
        ...
        successOutput: (result: unknown) => {
          const created = JSON.parse(String(result)) as { path?: unknown };
