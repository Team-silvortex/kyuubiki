...
type ProjectBundleSummary = {
  ...
  jobCount?: unknown;
  resultCount?: unknown;
  analysisDomains?: unknown;
  analysisFamilies?: unknown;
  thermalIntents?: unknown;
};

type HubWorkloadRuntimeContext = {
  downloadHubBlob: (filename: string, blob: Blob) => void;
  downloadHubJson: (filename: string, payload: unknown) => void;
  elements: HubWorkloadElements;
  ...
  projectSummaryFromInspectPayload: (raw: string) => ProjectBundleSummary;
...
function asEntryList(entries: unknown[]): HubWorkloadEntry[] {
  return entries.filter((entry): entry is HubWorkloadEntry => Boolean(entry));
}
...
function markHubWorkloadDownloaded(context: HubWorkloadRuntimeContext, entry: HubWorkloadEntry): void {
...
export function updateHubWorkloadEntry(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
  updater: (entry: HubWorkloadEntry) => unknown,
): void {
  const next = context.loadHubWorkloadLibrary()
    ...
    .filter((candidate): candidate is HubWorkloadEntry => Boolean(candidate));
  context.saveHubWorkloadLibrary(next);
}

export async function downloadRemoteWorkloadBundle(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
): Promise<void> {
...
export async function openWorkloadInWorkbench(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
): Promise<void> {
  if (!entry.bundlePath) {
...
export async function attachCurrentBundleToWorkload(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
): Promise<void> {
...
export function saveHubWorkloadLibrary(
  context: HubWorkloadRuntimeContext,
  entries: HubWorkloadEntry[],
): void {
  context.persistHubWorkloadLibrary(entries);
  context.renderHubWorkloadLibrary(entries);
}

export function matchesWorkloadFamilyFilter(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
): boolean {
...
export function matchesWorkloadFilter(context: HubWorkloadRuntimeContext, entry: HubWorkloadEntry): boolean {
...
export function currentWorkloadLibrarySearchQuery(context: HubWorkloadRuntimeContext): string {
  return String(context.elements.workloadLibrarySearch?.value || "").trim().toLowerCase();
}

export function matchesWorkloadSearchQuery(
  context: HubWorkloadRuntimeContext,
  entry: HubWorkloadEntry,
): boolean {
...
export async function registerCurrentBundleAsWorkload(context: HubWorkloadRuntimeContext): Promise<void> {
  const bundlePath = String(context.elements.projectBundlePath?.value || "").trim();
  if (!bundlePath) {
...
export async function syncRemoteWorkloadCatalog(
  context: HubWorkloadRuntimeContext,
  urlOverride = "",
): Promise<void> {
  const selectedUrl =
    String(urlOverride || "").trim() || String(context.elements.workloadCatalogUrl?.value || "").trim();
...
export async function syncLocalControlPlaneWorkloads(context: HubWorkloadRuntimeContext): Promise<void> {
  const catalogUrl = context.ensureDefaultWorkloadCatalogUrl(true);
  await syncRemoteWorkloadCatalog(context, catalogUrl);
}

export function exportHubWorkloadLibrary(context: HubWorkloadRuntimeContext): void {
  const payload = {
...
export async function importHubWorkloadLibrary(
  context: HubWorkloadRuntimeContext,
  file?: File | null,
): Promise<void> {
  if (!file) {
...
export function clearHubWorkloadLibrary(context: HubWorkloadRuntimeContext): void {
