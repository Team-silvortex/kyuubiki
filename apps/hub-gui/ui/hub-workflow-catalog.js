...
function scoreWorkflowCatalogEntry(entry, tokens) {
    ...
    const score = tokens.reduce((total, token) => {
        const best = metadata.weightedFields.reduce((max, [field, weight]) => {
            if (field === token)
                return Math.max(max, weight * 6);
            if (field.startsWith(token))
                return Math.max(max, weight * 4);
...
export async function waitForWorkflowJob(jobId, options) {
    ...
    while (Date.now() - startedAt < timeoutMs) {
        ...
        if (status === "failed" || status === "cancelled") {
            throw new Error(options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogFailed"), {
                workflow: result.workflow_id || job.job_type || "workflow",
                status,
            }));
        }
        options.setWorkflowCatalogOutput(options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogPolling"), {
            job: jobId,
        }));
...
export async function runWorkflowCatalogSample(entry, options) {
    ...
    if (!workflowId || !inputArtifacts) {
        options.setWorkflowCatalogOutput(options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogUnsupported"), {
    ...
    try {
        const response = await fetch(`${options.currentWorkflowCatalogUrl().replace(/\/+$/u, "")}/${encodeURIComponent(workflowId)}/jobs`, {
            method: "POST",
            headers: {
        ...
        const queuedMessage = options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogQueued"), {
            workflow: workflowId,
            job: jobId || "--",
        });
        ...
        const completedNodes = Array.isArray(result.completed_nodes)
            ...
            : [];
        const completedMessage = options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogCompleted"), {
            workflow: workflowId,
        ...
        options.setWorkflowCatalogOutput(options.formatHubOperatorError(error, {
...
export async function fetchWorkflowCatalog(options) {
    ...
    try {
        ...
        options.state.workflowCatalog = Array.isArray(workflows) ? workflows : [];
        options.renderWorkflowCatalog();
        if (!silent) {
            const loadedMessage = options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogLoaded"), {
...
export function renderWorkflowCatalog(entries, options) {
    ...
    if (!entries.length) {
        options.renderEmptyHistoryState(workflowCatalogList, options.localizedWorkflowCatalogLabel("workflowCatalogEmpty"));
        return;
    }
    ...
    if (query && !suggestedEntries.length) {
        options.renderEmptyHistoryState(workflowCatalogList, options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogNoSearchMatches"), {
            query,
        }));
        return;
    }
    suggestedEntries.forEach(({ entry, score, matchedTerms }) => {
        ...
        if (query) {
            options.appendTextElement(meta, "span", options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogSuggestedBadge"), {
                score,
            }), "desktop-shell-chip");
        }
        ...
        if (query && matchedTerms.length) {
            options.appendTextElement(summary, "span", options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogMatchedTerms"), {
                terms: matchedTerms.join(", "),
            }), "hub-history-item__alias");
        }
        options.appendTextElement(summary, "span", options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogEntryInputs"), {
            inputs: Object.keys(entry.entry_inputs || {}).join(", ") || "--",
        }), "hub-history-item__alias");
        options.appendTextElement(summary, "span", options.hubMessage(options.localizedWorkflowCatalogLabel("workflowCatalogOutputs"), {
