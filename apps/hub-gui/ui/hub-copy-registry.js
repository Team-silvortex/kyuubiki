...
function normalizeHubCopyImportManifest(payload) {
  ...
  normalized.mode = ["none", "pack", "registry"].includes(mode) ? mode : "none";
  ...
  normalized.packs = Array.isArray(payload.packs)
    ? payload.packs.map((entry) => normalizePackDescriptor(entry)).filter(Boolean)
    : [];
  return normalized;
}
...
export function importHubCopyPayload(payload, storage = window.localStorage) {
  ...
  if (
    payload?.schema_version === HUB_LANGUAGE_PACK_SCHEMA_VERSION ||
    (typeof payload?.language === "string" && isPlainObject(payload?.overrides))
  ) {
    const descriptor = buildLanguagePackDescriptor(payload);
    if (!descriptor) {
    ...
    const nextRegistry = normalizeHubCopyOverrideRegistry({
      ...
      languages: {
        ...
        [descriptor.language]: payload.overrides,
      },
    });
  ...
  saveHubCopyImportManifest(
    {
      mode: "registry",
      importedAt,
      registryLabel: typeof payload?.name === "string" ? payload.name : "",
      packs: [],
    },
    storage,
  );
