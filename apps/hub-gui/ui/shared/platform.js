...
export function normalizeDesktopPlatform(value, fallback = detectDesktopPlatform(), options = {}) {
  ...
  return DESKTOP_PLATFORMS.includes(value) ? value : fallback;
}

export function desktopPlatformLabel(value) {
  return DESKTOP_PLATFORM_LABELS[normalizeDesktopPlatform(value, "linux", { allowAll: true })] || DESKTOP_PLATFORM_LABELS.linux;
}
...
export function desktopPlatformOptions(options = {}) {
  ...
  return entries.map((value) => ({
    value,
    label: DESKTOP_PLATFORM_LABELS[value] || value,
  }));
}
...
export function syncDesktopReleaseTargetInput(input, platform, options = {}) {
  if (!input) return;
