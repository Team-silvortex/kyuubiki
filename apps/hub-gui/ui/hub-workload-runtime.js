...
export async function downloadRemoteWorkloadBundle(context, entry) {
  ...
  if (!context.ensureRemoteHostTrust(validation.normalized, "This workload download")) {
  ...
  const filename = context.inferDownloadFilename(validation.normalized);
...
export async function syncRemoteWorkloadCatalog(context, urlOverride = "") {
  ...
  if (context.elements.workloadCatalogUrl) {
    context.elements.workloadCatalogUrl.value = validation.normalized;
  }

  if (!context.ensureRemoteHostTrust(validation.normalized, "This remote catalog sync")) {
  ...
  const normalized = context.normalizeRemoteWorkloadCatalogPayload(payload, validation.normalized);
...
export async function importHubWorkloadLibrary(context, file) {
  ...
  const normalized = imported
    .map((entry) =>
      context.normalizeHubWorkloadEntry({
        ...entry,
