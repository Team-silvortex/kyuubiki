...
export function renderHubLocalizationPanel({ elements, copy, activeLanguage, setText }) {
  ...
  if (!elements.guidesLocalizationOutput?.textContent?.trim() || elements.guidesLocalizationOutput.dataset.localizationOutputKind === "idle") {
    elements.guidesLocalizationOutput.textContent =
      languageKeys.length > 0 || defaultLayerEnabled ? copy.guides.localizationOutputReady : copy.guides.localizationOutputEmpty;
    elements.guidesLocalizationOutput.dataset.localizationOutputKind = "idle";
  }
...
export function bindHubLocalizationPanel(params) {
  ...
  const setOutput = (value, kind = "status") =>
