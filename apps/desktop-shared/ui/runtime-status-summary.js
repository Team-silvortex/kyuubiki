...
function applySelectionDecorators(model) {
  ...
  return {
    ...model,
    runtimes: (Array.isArray(model.runtimes) ? model.runtimes : []).map((runtime) => ({
      ...runtime,
      selectionKey: `runtime:${runtime.id}`,
      isSelected: selectedKey === `runtime:${runtime.id}`,
    })),
    meshPanels: (Array.isArray(model.meshPanels) ? model.meshPanels : []).map((panel, panelIndex) => ({
      ...panel,
      selectionKey: panel.detail ? `panel:${panelIndex}` : "",
      isSelected: selectedKey === `panel:${panelIndex}`,
      items: (Array.isArray(panel.items) ? panel.items : []).map((item, itemIndex) => {
    ...
    topology: (Array.isArray(model.topology) ? model.topology : []).map((section, sectionIndex) => ({
      ...section,
...
function normalizeRenderInputs(payload, options = {}) {
  const nestedSummary =
    ...
    typeof payload.summary === "object"
      ...
      : null;
  const healthPayload =
    options && typeof options === "object" && !("onFilterChange" in options) && ("solver_agents" in options || "remote_solver_registry" in options || "deployment" in options)
      ? options
  ...
  if (summaryPayload || healthPayload) {
    ...
    const selectedFilter =
      options && typeof options === "object" && typeof options.selectedFilter === "string"
        ? options.selectedFilter
        : model.selectedFilter;
    const selectedDetail =
      options && typeof options === "object" && typeof options.selectedDetail === "string"
    ...
    return {
      ...
      options:
        options && typeof options === "object" && ("onFilterChange" in options || "healthPayload" in options)
          ? options
          : {},
    };
  }
