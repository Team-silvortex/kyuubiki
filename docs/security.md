...
| `workers/rust/crates/desktop-runtime/src/direct_mesh_gateway.rs` | Token gate, endpoint allowlist, and bounded result cache for browser-facing direct-mesh routes. | Keep hosted deployments opt-in, compare tokens without early exit, and reject request-defined endpoints unless explicitly enabled. |
| `workers/rust/crates/desktop-runtime/src/direct_mesh_rpc.rs` | Native TCP bridge to solver agents selected by the runtime gateway. | Endpoint validation, connect/read timeouts, request ID matching, RPC method allowlists, and frame-size bounds. |
| `workers/rust/crates/desktop-runtime/src/frontend_http.rs` | Installed static asset and local HTTP request boundary. | Path traversal, body/header limits, MIME safety, no directory listing, and loopback-only serving. |
