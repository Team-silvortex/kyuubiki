...
from `tamamono 1.6.x` into `tamamono 1.7.x`.
...
Before the repo claims `1.7.0`, we want three things to be true:
...
1. Keep the shipping line at `1.6.0` until the 1.7 contract is ready.
2. Run the audit and clear any 1.6 drift that already exists.
3. Create the `1.7.0` snapshot scaffold:

```bash
node ./scripts/create-release-snapshot.mjs 1.7.0 --status staged --dry-run
```
