...
- `make architecture-check`
  Lightweight new-architecture guard for the `1.17.x` line. It runs the
...
- `make check-operator-reliability`
  ...
  `minimum_coverage_level`, currently `review` for the `tamamono 1.17.x`
...
- `make benchmark-physics-coverage`
  Run the `1.17.x` broad physics smoke matrix across every built-in benchmark
  template. This is the quickest product-level check that the main physics
  families still have real solver execution paths before `1.17.x` and `1.17.x`
