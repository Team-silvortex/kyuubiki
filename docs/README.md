...
For the current `1.17.x` hardening path, keep four threads mentally linked:
...
- `physics-coverage-map.md`
  `1.17.x` coverage map for broad solver-family execution coverage and the
  review-level reliability gate before the `1.17.x` and `1.17.x`
...
- `installer-remote-control.md`
  ...
  control-surface note for the `1.17.x` preparation line.
...
- `project-architecture-organization.md`
  Current `1.17.x` architecture organization map for Hub, Workbench,
...
- `installer-remote-control.md`
  ...
  inside the `1.17.x` trust-hardening and asset-formalization path.
- `component-integrity-protocol.html`
