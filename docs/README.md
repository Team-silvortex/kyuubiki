...
- `physics-coverage-map.md`
  `1.18.x` coverage map for broad solver-family execution coverage and the
  review-level reliability gate before the `1.18.x` and `1.18.x`
...
- `installer-remote-control.md`
  ...
  control-surface note for the `1.18.x` preparation line.
...
- `project-architecture-organization.md`
  Current `1.18.x` architecture organization map for Hub, Workbench,
...
- `installer-remote-control.md`
  ...
  inside the `1.18.x` trust-hardening and asset-formalization path.
- `component-integrity-protocol.html`
