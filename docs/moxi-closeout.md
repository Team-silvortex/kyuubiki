# Historical Moxi Closeout

Retained at the daji 3.0.0 transition. This is historical scope, not the active
release identity or fresh Daji qualification. See [current-line.md](current-line.md).
...
surfaces, central-store contracts, material exploration chains, operator model
drafts, and repository path containment.
...
- runtime API verification now binds the Protocol and all three official
  ...
  retained 33/33 cross-shell and live Workbench interaction report, including
  controlled Model/Study, System, Library, Runtime, and Data round trips plus
  stale Store response rejection, shared GUI/PWDT Store manifest commands, and
  no-click PWDT Store/Workflow navigation; the
