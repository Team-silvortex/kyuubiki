...
use kyuubiki_protocol::{
    SolveCompositeThermoElectricPanelRequest, SolveElectrostaticPlaneQuad2dRequest,
...
pub(crate) fn run_solve_step(step: &HeadlessWorkflowStep) -> Result<Value, String> {
    if step.action == "solve_composite_thermo_electric_panel" {
        let request: SolveCompositeThermoElectricPanelRequest =
            serde_json::from_value(step.payload.clone()).map_err(|error| error.to_string())?;
        let mut result = kyuubiki_cli::solve_composite_thermo_electric_panel(&request)?;
