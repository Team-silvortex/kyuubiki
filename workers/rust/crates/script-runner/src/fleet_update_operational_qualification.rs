...
use kyuubiki_installer::{
    FleetUpdateQualificationReport, FleetUpdateQualificationSummary,
...
pub(crate) fn run_check(root: &Path, args: Vec<OsString>) -> RunnerResult<u8> {
    ...
    println!(
        "Fleet update operational qualification passed: {} -> {} -> {}, {} Agents, {} probes, {}",
...
pub(crate) fn run_qualify_remote(root: &Path, args: Vec<OsString>) -> RunnerResult<u8> {
    ...
    match (capture, cleanup) {
        (Ok(0), Ok(())) => {
            println!(
                "remote Installer-managed fleet update qualification passed: {}",
...
impl RemoteOptions {
    fn parse(root: &Path, args: Vec<OsString>) -> RunnerResult<Self> {
        ...
        let slug = format!("fleet-update-{}", utc_timestamp_slug());
        let mut options = Self {
            help: false,
            host: env::var("KYUUBIKI_LAB_HOST").unwrap_or_else(|_| "kyuubiki-lab".into()),
        ...
        while let Some(arg) = args.next() {
            match arg.to_string_lossy().as_ref() {
                ...
                "--agents" => {
                    options.agent_count = next_string(&mut args, "--agents")?
                        .parse()
                        .map_err(|_| "--agents must be an integer".to_string())?;
                }
        ...
        if !(2..=8).contains(&options.agent_count) {
            return Err("remote fleet qualification requires 2 to 8 Agents".into());
        }
...
fn remote_qualification_command(options: &RemoteOptions) -> String {
    ...
    let agent_count = options.agent_count;
    format!(
        "set -eu; umask 077; run_root={run_root}; source_root=\"$run_root/workers/rust\"; \
target_root=\"$HOME/.kyuubiki/cache/cargo-target/fleet-update-operational\"; \
evidence_root=\"$HOME/.kyuubiki/lab-evidence/fleet-update-operational\"; \
...
\"$target_root/release/kyuubiki-installer\" qualify-fleet-update \
\"$target_root/debug/kyuubiki-cli\" \"$target_root/release/kyuubiki-cli\" \
\"$run_root/qualification-work\" {first} {second} \"$run_root/report.json\" {agent_count}; \
...
fn verify_report(
    path: &Path,
    require_remote_linux: bool,
) -> RunnerResult<FleetUpdateQualificationSummary> {
    let bytes =
        fs::read(path).map_err(|error| format!("failed to read {}: {error}", path.display()))?;
    let report: FleetUpdateQualificationReport = serde_json::from_slice(&bytes)
        .map_err(|error| format!("{}: invalid report: {error}", path.display()))?;
    let summary = validate_fleet_update_qualification_report(&report)
...
fn default_report_path(root: &Path, version: &str) -> PathBuf {
    root.join(format!(
        "releases/usability-evidence/{version}/fleet-update-operational-qualification.json"
    ))
}
...
fn temporary_report_path(output: &Path) -> PathBuf {
    let file = output
        ...
        .unwrap_or("fleet-update-operational.json");
...
fn print_check_usage() {
    println!(
        "usage: kyuubiki-script-runner check-fleet-update-operational-qualification [--verify-report path] [--require-remote-linux]"
    );
}

fn print_remote_usage() {
    println!(
        "usage: kyuubiki-script-runner qualify-fleet-update-operational-remote [--host SSH_ALIAS] [--out path] [--first-version version] [--second-version version] [--agents count]"
    );
}
...
mod tests {
    ...
    fn derives_previous_patch_across_minor_boundaries() {
        assert_eq!(previous_version("2.18.1").unwrap(), "2.18.0");
        assert_eq!(previous_version("2.18.0").unwrap(), "2.17.9");
