...
#[path = "material_research_bundle_contract_schema.rs"]
mod material_research_bundle_contract_schema;
...
fn check_contracts(root: &Path) -> RunnerResult<Vec<String>> {
    ...
    material_research_bundle_contract_schema::check_schema(
        &read_json(root, SCHEMA_PATH)?,
        &mut issues,
    );
