...
pub(crate) fn run_qualify_remote(root: &Path, args: Vec<OsString>) -> RunnerResult<u8> {
    ...
    let (phases, cleanup) = runtime::capture(
        root,
        &options.host,
        Duration::from_secs(options.timeout_seconds),
    )?;
    let qualification = report::build_report(phases, cleanup)?;
    report::validate(root, &qualification)?;
    report::write(root, &options.output, &qualification)?;
    println!(
        "two-host Agent control-link qualification passed: {}",
        options.output
    );
    Ok(0)
}
...
pub(crate) fn run_check(root: &Path, args: Vec<OsString>) -> RunnerResult<u8> {
    ...
    if options.self_test {
        ...
        println!("Agent control-link operational qualification self-test passed");
    ...
    report::validate(root, &qualification)?;
    println!("Agent control-link operational qualification report passed: {report_path}");
    Ok(0)
}
    ...
    fn parse(args: Vec<OsString>) -> RunnerResult<Self> {
        ...
        while let Some(arg) = iter.next() {
            match arg.to_string_lossy().as_ref() {
                ...
                other => return Err(format!("unknown control-link capture option: {other}")),
            }
        }
        ...
        if !(15..=180).contains(&options.timeout_seconds) {
            return Err("--timeout-secs must be between 15 and 180".to_string());
        }
        Ok(options)
    }
}
    ...
    fn parse(args: Vec<OsString>) -> RunnerResult<Self> {
        ...
        while let Some(arg) = iter.next() {
            match arg.to_string_lossy().as_ref() {
                ...
                other => return Err(format!("unknown control-link check option: {other}")),
            }
        }
        Ok(options)
    }
...
fn print_capture_usage() {
    println!(
        "usage: kyuubiki-script-runner qualify-agent-control-link-operational-remote [--host SSH_ALIAS] [--out path] [--timeout-secs seconds]"
    );
}

fn print_check_usage() {
    println!(
        "usage: kyuubiki-script-runner check-agent-control-link-operational-qualification [--self-test] [--verify-report path]"
    );
}
...
mod tests {
    ...
    #[test]
    fn capture_options_reject_path_escape_and_ssh_option_injection() {
    ...
    fn capture_timeout_is_bounded() {
        assert!(CaptureOptions::parse(vec!["--timeout-secs".into(), "14".into()]).is_err());
        assert!(CaptureOptions::parse(vec!["--timeout-secs".into(), "60".into()]).is_ok());
    }
}
