...
use kyuubiki_installer::{
    AgentUpdateQualificationReport, AgentUpdateQualificationSummary,
    validate_agent_update_qualification_report,
};
...
pub(crate) fn run_check_agent_update_operational_qualification(
    root: &Path,
    args: Vec<OsString>,
) -> RunnerResult<u8> {
    ...
    println!(
        "Agent update operational qualification passed: {} -> {} -> {}, {}",
...
pub(crate) fn run_qualify_agent_update_operational_remote(
    root: &Path,
    args: Vec<OsString>,
) -> RunnerResult<u8> {
    ...
    match (capture, cleanup) {
        (Ok(0), Ok(())) => {
            println!(
                "remote Installer-managed Agent update qualification passed: {}",
...
impl RemoteOptions {
    fn parse(root: &Path, args: Vec<OsString>) -> RunnerResult<Self> {
        ...
        let slug = format!("agent-update-{}", utc_timestamp_slug());
        let mut options = Self {
            help: false,
...
target_root=\"$HOME/.kyuubiki/cache/cargo-target/agent-update-operational\"; \
evidence_root=\"$HOME/.kyuubiki/lab-evidence/agent-update-operational\"; \
...
\"$target_root/debug/kyuubiki-installer\" qualify-agent-update \
...
fn verify_report(
    path: &Path,
    require_remote_linux: bool,
) -> RunnerResult<AgentUpdateQualificationSummary> {
    let bytes =
        fs::read(path).map_err(|error| format!("failed to read {}: {error}", path.display()))?;
    let report: AgentUpdateQualificationReport = serde_json::from_slice(&bytes)
        .map_err(|error| format!("{}: invalid report: {error}", path.display()))?;
    let summary = validate_agent_update_qualification_report(&report)
...
fn default_report_path(root: &Path, version: &str) -> PathBuf {
    root.join(format!(
        "releases/usability-evidence/{version}/agent-update-operational-qualification.json"
    ))
}
...
fn temporary_report_path(output: &Path) -> PathBuf {
    let file = output
        ...
        .unwrap_or("agent-update-operational.json");
...
fn print_check_usage() {
    println!(
        "usage: kyuubiki-script-runner check-agent-update-operational-qualification [--verify-report path] [--require-remote-linux]"
    );
}

fn print_remote_usage() {
    println!(
        "usage: kyuubiki-script-runner qualify-agent-update-operational-remote [--host SSH_ALIAS] [--out path] [--first-version version] [--second-version version]"
    );
}
