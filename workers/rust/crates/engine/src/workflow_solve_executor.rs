...
pub fn run_solve_operator(operator_id: &str, payload: Value) -> Result<Value, String> {
    match operator_id {
        ...
        "solve.cohesive_interface_1d" => encode_solve_result(
        ...
        "solve.cohesive_interface_mesh_2d" => encode_solve_result(
            solve(EngineSolveRequest::CohesiveInterfaceMesh2d(decode(
                payload,
            )?))?,
            |result| match result {
        ...
        "solve.frame_2d_p_delta" => encode_solve_result(
            solve(EngineSolveRequest::Frame2dPDelta(decode(payload)?))?,
            |result| match result {
                AnalysisResult::Frame2dPDelta(result) => Some(result),
                _ => None,
            },
            operator_id,
        ),
        "solve.frame_2d_p_delta_path" => encode_solve_result(
            solve(EngineSolveRequest::Frame2dPDeltaPath(decode(payload)?))?,
            |result| match result {
                AnalysisResult::Frame2dPDeltaPath(result) => Some(result),
                _ => None,
            },
            operator_id,
        ),
        "solve.frame_2d_material_p_delta" => encode_solve_result(
            solve(EngineSolveRequest::Frame2dMaterialPDelta(decode(payload)?))?,
            |result| match result {
                AnalysisResult::Frame2dMaterialPDelta(result) => Some(result),
                _ => None,
            },
            operator_id,
        ),
