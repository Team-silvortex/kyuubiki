...
pub fn solve(request: EngineSolveRequest) -> Result<AnalysisResult, String> {
    match request {
        ...
        EngineSolveRequest::CohesiveInterface2d(request) => {
            solve_cohesive_interface_2d(&request).map(AnalysisResult::CohesiveInterface2d)
        }
        EngineSolveRequest::CohesiveInterfaceMesh2d(request) => {
            solve_cohesive_interface_mesh_2d(&request).map(AnalysisResult::CohesiveInterfaceMesh2d)
        }
        ...
        EngineSolveRequest::BucklingFrame2d(request) => {
            solve_buckling_frame_2d(&request).map(AnalysisResult::BucklingFrame2d)
        }
        EngineSolveRequest::Frame2dPDelta(request) => {
            solve_frame_2d_p_delta(&request).map(AnalysisResult::Frame2dPDelta)
        }
