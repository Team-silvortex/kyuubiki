...
pub(crate) fn bridge_electrostatic_result_to_heat_plane_quad_model(
    ...
    for (index, heat_node) in bridged.nodes.iter_mut().enumerate() {
        let value = match counts[index] {
            0 => contract.default_value,
            _ if contract.reduction == "sum" => sums[index],
            count => sums[index] / count as f64,
        };
...
fn electrostatic_bridge_source_value(
    ...
    match field {
        ...
        "electric_flux_density_magnitude" => Ok(element.electric_flux_density_magnitude),
