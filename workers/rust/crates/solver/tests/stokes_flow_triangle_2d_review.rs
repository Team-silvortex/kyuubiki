...
#[test]
fn stokes_flow_triangle_2d_review_bundle_checks_linear_gradient_metrics() {
    let viscosity = 2.0;
    let density = 1.0;
    let thickness = 0.1;
    let result = solve_stokes_flow_plane_triangle_2d(&SolveStokesFlowPlaneTriangle2dRequest {
        nodes: vec![
            ...
            node("n2", 0.0, 1.0, true, 0.0, false, 0.0, false, 0.0, 0.0, 0.5),
        ],
    ...
    .expect("review stokes flow triangle should solve");

    assert_close(result.nodes[1].velocity_x, 1.0);
    assert_close(result.nodes[2].velocity_y, 0.25);
    assert_close(result.nodes[1].pressure, -2.0);
    assert_close(result.nodes[2].pressure, -0.5);

    let average_velocity_x = 1.0 / 3.0;
    let average_velocity_y = 0.25 / 3.0;
    let average_velocity_magnitude = f64::sqrt(
        average_velocity_x * average_velocity_x + average_velocity_y * average_velocity_y,
    );
    let average_pressure = -0.5;
    let du_dx = 1.0;
    let dv_dy = 0.25;
    let shear_rate = f64::sqrt(2.0 * du_dx * du_dx + 2.0 * dv_dy * dv_dy);
    let reynolds_number = density * average_velocity_magnitude * f64::sqrt(0.5) / viscosity;
    let viscous_dissipation = viscosity * (du_dx * du_dx + dv_dy * dv_dy) * 0.5 * thickness;
    ...
    assert_close(element.velocity_gradient_y, 0.0);
    assert_close(element.shear_rate, shear_rate);
    assert_close(element.max_viscous_shear_stress, viscosity * 2.0);
    assert_close(element.divergence_error, du_dx + dv_dy);
    ...
    assert_close(result.max_pressure, 2.0);
    assert_close(result.pressure_drop, 3.0);
    assert_close(result.max_divergence_error, du_dx + dv_dy);
    assert_close(result.max_reynolds_number, reynolds_number);
    assert_close(result.max_shear_rate, shear_rate);
    assert_close(result.max_viscous_shear_stress, viscosity * 2.0);
}
