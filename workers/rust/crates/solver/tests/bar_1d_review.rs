...
fn bar_1d_review_bundle_exposes_assumptions_boundaries_and_diagnostics() {
    ...
    let expected_energy_density =
        0.5 * expected_stress * (expected_stress / request.youngs_modulus);
    let expected_total_energy = 0.5 * request.tip_force * expected_tip_displacement;
