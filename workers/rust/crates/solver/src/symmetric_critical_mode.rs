...
pub(crate) fn extract_symmetric_critical_modes(
    ...
    if normalized.iter().flatten().any(|value| !value.is_finite()) {
        return Err("critical-mode tangent contains a non-finite value".to_string());
    }
    ...
    let mut eigenpairs = jacobi_eigenpairs(normalized.clone())?;
...
mod tests {
    ...
    fn extracts_an_ordered_orthogonal_critical_subspace() {
        ...
        let modes = extract_symmetric_critical_modes(&matrix, &[0, 1, 2], 3, 2).unwrap();
