...
fn validate_lineage(
    ...
    if spec.iteration == 1 {
        ...
        validate_receipt_target(batch, batch_content_sha256, patch_receipt)?;
        return Ok(None);
    }
...
mod tests {
    use super::*;
    use crate::{
        HEADLESS_PARAMETER_PATCH_SCHEMA_VERSION, HeadlessExecutionBatchStep,
        HeadlessParameterChange, HeadlessParameterPatch, HeadlessRisk, apply_parameter_patch,
        run_batch_dry,
    };
    use serde_json::json;
    ...
    fn qualifies_contiguous_rounds_with_changed_input_and_numeric_metrics() {
        ...
        let receipt = apply_parameter_patch(
            &mut second_batch,
