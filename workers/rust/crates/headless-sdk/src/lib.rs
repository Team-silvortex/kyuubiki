...
pub fn validate_batch(batch: &HeadlessExecutionBatch) -> HeadlessValidationReport {
    ...
    let policy = Some(build_policy_summary(batch));
    let summary = Some(summarize_batch(batch));
...
mod tests {
    ...
    fn rejects_future_binding_reference() {
        let mut document = build_template_document("workflow_submit_monitor", None).unwrap();
