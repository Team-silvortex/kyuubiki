...
use crate::material_composite_models::{
    composite_research_metadata, electric_conduction_feedback_model, electric_conduction_model,
    electrostatic_model, electrothermal_feedback_model, electrothermal_loss_model, heat_model,
    thermal_expansion_feedback_model, thermal_model,
};
use crate::material_composite_quality::{
    composite_coupling_quality_gates, composite_structural_quality_gates,
};
use crate::{
    CompositeCurrentToHeatProjection, CompositeElectrostaticCrossValidation,
    CompositeElectrostaticMeshConvergence, CompositeElectrothermalFeedbackConvergence,
    ...
    CompositeThermalConstraintSensitivity, CompositeThermalExpansionProjection,
    CompositeThermalInterfaceGradingAssessment, CompositeThermalMeshConvergence,
    CompositeThermalStressRecovery, HeadlessWorkflowStep, MaterialCardReference,
    ...
    composite_electrostatic_mesh_convergence, composite_heat_cross_validation,
    composite_heat_cross_validation_for_distributed_load, composite_heat_mesh_convergence,
    composite_heat_mesh_convergence_for_distributed_load, composite_thermal_constraint_sensitivity,
    composite_thermal_interface_grading_assessment, composite_thermal_mesh_convergence,
    composite_thermal_stress_recovery, material_optimization_constraint,
    material_optimization_profile, material_optimization_term, material_optimization_weight,
    material_quality_gate, material_reliability_summary, profile_weight,
};
...
pub struct CompositePanelCandidateReport {
    ...
    pub joule_heating_projection: Option<CompositeCurrentToHeatProjection>,
    pub electrothermal_feedback_convergence: Option<CompositeElectrothermalFeedbackConvergence>,
    ...
    pub thermal_expansion_projection: Option<CompositeThermalExpansionProjection>,
        ...
        .map(|candidate| {
            HeadlessWorkflowStep::new(
                ...
                json!({
                    ...
                    "electric_conduction_model": electric_conduction_model(&candidate),
                    "electric_conduction_feedback": electric_conduction_feedback_model(&candidate),
                    "heat_model": heat_model(&candidate),
                    "thermal_model": thermal_model(&candidate),
                    "thermal_expansion_feedback": thermal_expansion_feedback_model(),
                }),
            )
        })
        .collect()
...
pub fn build_composite_panel_report(
    ...
    Ok(CompositePanelReport {
        ...
        coupling: "iterative_dielectric_and_conductor_joule_heating_with_temperature_dependent_electrical_thermal_feedback_and_expansion_projection".to_string(),
...
fn composite_candidate_report(
    ...
    let electrothermal_feedback_convergence = result
    ...
    let electrostatic_cross_validation = electrothermal_loss_projection.as_ref().map_or_else(
        || composite_electrostatic_cross_validation(candidate, max_electric_field_v_m),
        |projection| {
            composite_electrostatic_cross_validation_for_dielectric(
                projection.relative_permittivity,
                max_electric_field_v_m,
            )
        },
    );
    ...
    if electrothermal_feedback_convergence.is_none() {
        missing_metrics.push("electrothermal_feedback_convergence".to_string());
    }
    if joule_heating_projection.is_none() {
        missing_metrics.push("joule_heating_projection".to_string());
    }
    if heat_to_thermal_projection.is_none() {
        missing_metrics.push("heat_to_thermal_projection".to_string());
    }
    if thermal_expansion_projection.is_none() {
        missing_metrics.push("thermal_expansion_projection".to_string());
    }
    CompositePanelCandidateReport {
...
fn composite_quality_gates(rows: &[CompositePanelCandidateReport]) -> Vec<MaterialQualityGate> {
    ...
    gates.extend(composite_coupling_quality_gates(rows));
