...
const ORCH_ID = "orch-integration";
const ORCH_SESSION_ID = "session-distributed-a";
const AGENTS = [
  {
    id: "integration-remote-a",
    port: 6501,
    fingerprint: "sha256:integration-remote-a",
  },
  {
    id: "integration-remote-b",
    port: 6502,
    fingerprint: "sha256:integration-remote-b",
  },
];
...
function startRemoteAgent(agent) {
  ...
  const logPath = path.join(RUN_DIR, `integration-remote-agent-${agent.port}.log`);
...
function buildDistributedHeatToThermoGraph(passThroughCount, thermoSeedModel) {
  ...
  return {
    ...
    id: `workflow.distributed-heat-to-thermo-chain-${passThroughCount}`,
    name: "Distributed heat to thermo chain",
    version: "1.0.0",
...
test("distributed orchestrator can run a complex coupled workflow through registered remote agents", async () => {
  ...
  try {
    ...
    assert.equal(agentsPayload.summary.control_modes.orch_managed, 2);
    assert.equal(agentsPayload.summary.control_modes.offline_mesh, 0);
    ...
    const submitResponse = await fetch(`${ORCHESTRATOR_URL}/api/v1/workflows/graph/jobs`, {
      ...
      headers: controlHeaders(),
      body: JSON.stringify({
        graph: buildDistributedHeatToThermoGraph(16, thermoSeedModel),
        input_artifacts: {
    ...
    assert.equal(finalPayload.result.workflow_id, "workflow.distributed-heat-to-thermo-chain-16");
